...
-- Create ai_memories table first with all required fields
...
CREATE INDEX IF NOT EXISTS idx_ai_memories_embedding ON public.ai_memories USING ivfflat (embedding vector_cosine_ops);
CREATE INDEX IF NOT EXISTS idx_ai_memories_type ON public.ai_memories(memory_type, created_at);
...
CREATE POLICY "Admins can manage memories" ON public.ai_memories
  FOR ALL USING (is_admin(auth.uid()));

CREATE POLICY "System can access memories for processing" ON public.ai_memories
...
AS $$
  SELECT
    ...
    (ai_memories.embedding <=> query_embedding) * -1 + 1 AS similarity,
    ai_memories.relevance_score
  FROM public.ai_memories
  WHERE (ai_memories.embedding <=> query_embedding) * -1 + 1 > match_threshold
